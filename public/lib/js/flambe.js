...
init = () => {
  ...
  input.addEventListener('change', e => {
    if (targetSlot == null || input.files.length > 1) {
      ...
      return pBar(1, "READING...✓", "teal", 0.1, 0, 0).then(() => pBar(2, 'PARSING...✓', 'DarkTurquoise', 0.1, 0.1, 0.1)).then(() => addOrReplaceSingleFileAndParse());
    }
  });
};
...
const reFilterPreview = obj => {
  ...
  let totDisp = 0;
  [...qsa('.status-filter-checkboxes')].forEach(cb => {
    totDisp += cb.checked ? cb.value / 1 : 0;
  });
