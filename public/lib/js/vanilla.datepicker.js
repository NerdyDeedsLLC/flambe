(function (window, document) {
  ...
  datepicker.prototype = {
    ...
    drawNavigation: function () {
      ...
      currentMonth = document.createTextNode(self.options.months.long[self.current.month] + ' ' + self.current.year);
      ...
      nextMonth.innerHTML = '>'; //nextMonth.addEventListener('click', self.getNextMonth, false);
    ...
    getWeeks: function () {
      ...
      var weekdays = self.options.weekdays.short.slice(self.options.firstDayOfWeek).concat(self.options.weekdays.short.slice(0, self.options.firstDayOfWeek)); // Get first day of month and update acconding to options
      ...
      var days = [],
          weeks = []; // Define last days of previous month if current month does not start on `firstOfMonth`
