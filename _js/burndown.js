...
class BurndownChart {
    ...
    plotSpoilers(e, trg=e.target||null, scope){
        ...
        let y = trg.y.baseVal.value;
            x = Math.max(0,Math.min(700, x - 150))
            y = Math.max(0,Math.min(425, y * this.vertScaleFactor))
        ...
        spoilerAlert += `<text class="spoiler-ideal-label" x="${x + 173}" y="${y + 69}" text-anchor="start">ideal burn</text>
                        ...
                        <g transform="rotate(-90 ${x} ${y + 210})"><text class="spoiler-facilitated" x="${x + 10}" y="${y + 230}" text-anchor="start">facilitated by...</text></g>
        ...
        document.querySelectorAll("#didBurn, #didNotBurn").forEach(el=>el.addEventListener('click', this.toggleWhoBurnedView));
