...
init = () => {                                                                                      // ⓿ Initiate application, chaining steps 1-3 above to file input's onChange
    ...
    if(fileBuffer.length > 0) {                                                                         // If we DID manage to restore a previous buffer...
        desiredDays = retain('desiredDays', fileBuffer.length);                                         //    ... infer the # of days in the iteration from its number of indices.
        namedFiles  = retain('namedFiles', fileBuffer.flatMap(f=>(f && f.fileName) ?                    //    ... and, should it prove that we have a valid file for each (filled) index... ********
                                                                  ...
                                                                  'BUFFER ERROR'))                      //    ... otherwise, flag the individual record as having errored out.
    }
    ...
    if(fileBuffer.length === 0 || namedFiles.indexOf('BUFFER ERROR') !== -1){                           // If we FAILED to restore a previous buffer (or the one we DID errored out)...
        desiredDays = reflect('desiredDays', 10);                                                       //    ... 
...
//    new Promise(conclude => {
    {
        ...
        return new Promise(resolve => { // Entire addOrReplaceSingleFileAndParse returns this one promise, which should then chai9n below
                    _("Yay promises!");
                    ...
                    {            
                                _('ingestAndBufferFilesFromReader', fileObj, fileName);
                                            return new Promise(fulfill => {                                                                        // We're gonna do this all async-like, in case it's a big file.
                                            const appendToBuffer=(JSONObj, fileName = null) => {
                                                let newJSONData    = { fileName: fileName, fileData: JSONObj };
...
const processParentChildRelationships = () => {
    var toc = {};
    quickIndex = Object.entries(COMBD_DATA).flatMap(

        d=>{
            let nd = (d[1].flat()), dat=null;
            ...
            if(!dat) return null;
            console.log(dat);
            let obj={'issueKey':dat['Issue key'], 'jiraIssueId':dat['Issue id'], 'parent': dat['Parent id']}; 
            toc[obj.jiraIssueId] = obj.issueKey;
            obj['displayText'] = toc[obj.parent] ? toc[obj.parent] + ' / ' + obj.issueKey : obj.issueKey;
            obj['pathLinks'] = toc[obj.parent] ? '<a href="https://jira.sprintdd.com/browse/' + toc[obj.parent] + '" target="_blank" class="issue-parent-link">' + toc[obj.parent] + '</a>' + ' / ' + '<a href="https://jira.sprintdd.com/browse/' + obj.issueKey + '" target="_blank" class="issue-link">' + obj.issueKey + '</a>' : '<a href="https://jira.sprintdd.com/browse/' + obj.issueKey + '" target="_blank" class="issue-link">' + obj.issueKey + '</a>';
            // obj['pathLinks'] = obj.displayText.replace(/(DIGTDEV-\d+)/g, );
            return obj;
        });
        _(quickIndex, toc)
    
    quickIndex.toc = toc;
    quickIndex.getLinked = (issueID) => {return (quickIndex.find((f,i)=>f.jiraIssueId == issueID)).pathLinks || issueID};
    _(quickIndex)
    analyzeDataAndFlagConcerns();
    
}
const analyzeDataAndFlagConcerns = () => {                                                  // ⓺ iterate concatinated output data, look for concern-suggestive trends and build our markup
    ...
    Object.entries(COMBD_DATA).forEach((dataRecord, ind) => {                                   // Iterate across each Issue (the "rows") that we've ingested data for, to extract the following data:
        ...
        issueData.forEach((datRec, ix) => {                                                     // ...Iterate the issue's collected data (the "columns"), gathering...
            ...
            if(newRE !== oldRE) {                                                               // If the remaining hours have changed, we need to examine the delta to determine if this should be flagged
                flags = '';
    ...
    if (dateField.checkValidity()) {                                                            // Since we can't date-stamp a column if the user didn't give us a date, see if they did. IF they did...
        ...
        while (dateArr.length < 10 && dayCt < 50) {                                             // Keep going until we have at least 10 days
    ...
    let tblMarkup =             '<h1>' + iterationName.value + '</h1>' + 
        ...
        ,hdrMarkup =                '<thead><tr><th>' + 
                                        colHeaders.join('</th><th>')
        ,rowMarkup =                '<tbody>';
                                    '</th></thead></table>'
    MRKUP.forEach(o => rowMarkup +=    '<tr><td>' + 
                                            o.join('</td><td>').replace(/\.00h|\.0h/g, 'h') + 
                                        '</td></tr>');
    rowMarkup +=                   '</tbody>';
