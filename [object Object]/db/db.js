(function (factory) {
	...
	window.pogo = function (promiseToOutput) {
	  return Promise.resolve(promiseToOutput).then(promisaryResults => console.log('[POGO] - ', promiseToOutput, '=>', promisaryResults) || promisaryResults);
	};

	localforage.tables = null;
	localforage.getItem('tables').then(tblList => localforage.tables = new __TableStorage({
	  __parent: localforage,
	  __tables: tblList
	}));

	localforage.bindTableInstance = (instanceName, tableObject) => {
	  // let tableInstance;
	  Promise.resolve(() => localforage.createInstance({
	    name: instanceName
	  })).then(tableInstance => Object.assign(tableInstance, tableObject)).then(tableInstance => localforage.tables[instanceName] = tableInstance());
