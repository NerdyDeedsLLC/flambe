(function (factory) {
    typeof define === 'function' && define.amd ? define('asbestos', factory) :
    factory();
}((function () { 'use strict';
    ...
    class Asbestos {
      ...
      buildShitlist() {
        let sequences = {},
            assignees = [];
        fileBuffer.flatMap(r => r.fileData).forEach(issue => {
          if (issue == null || typeof issue["Issue key"] === "undefined") {
            return;
          }
        ...
        Object.values(sequences).forEach(seq => {
          var startingEstHours = seq[0]["Remaining Estimate"],
              hoursBurnedAtSomePoint = seq.some(day => day["Remaining Estimate"] < startingEstHours),
              finalAssignee = seq.reverse()[0]["Assignee"];
          seq.reverse();
          if (hoursBurnedAtSomePoint && assignees.indexOf(finalAssignee) !== -1) assignees.splice(assignees.indexOf(finalAssignee), 1);
        });
        let rval = assignees.length > 0 ? teamDialog.performSubstitution(`<li>${assignees.join("</li><li>")}</li>`) : ''; // console.log('rval :', rval);
...
//# sourceMappingURL=asbestos.js.map
